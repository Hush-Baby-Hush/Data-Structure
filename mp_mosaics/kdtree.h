...
class KDTree
{
  ...
  private:
     ...
     */
    int partition(vector<Point<Dim>>& Points, size_t left, size_t right, int curDim);
    Point<Dim> kthSmallest(vector<Point<Dim>>& Points, size_t left, size_t right, size_t k, int curDim);
    KDTreeNode* _findNearest(const Point<Dim>& query, KDTreeNode* currNode, KDTreeNode* bestNode, int curDim) const;
    void _destroy(KDTreeNode * subroot);
    double distance(Point<Dim> p1, Point<Dim> p2) const;
    KDTreeNode* _KDTree(vector<Point<Dim>> copyvector, int d);
