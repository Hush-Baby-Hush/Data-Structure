...
std::vector<Edge> NimLearner::playRandomGame() const {
 ...
 /* Your code goes here! */
  ...
  {
    Edge edge;
    vector<Vertex> vector_ = g_.getAdjacent(curr_);
    if(vector_.size() == 0) 
      break;
    if(vector_.size() == 1)
    {
      edge = g_.getEdge(curr_, vector_[0]);
      curr_ = vector_[0];
    }
    else
    {
      unsigned num = rand() % 2;
      edge = g_.getEdge(curr_, vector_[num]);
      curr_ = vector_[num];
    }
    path.push_back(edge);
  }
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
 /* Your code goes here! */
  ...
  Vertex curr_ = edg_.source.substr(0, 2);
  
  for(auto edge : path)
  {
    int weight = g_.getEdgeWeight(edge.source, edge.dest);
    if(curr_ == edge.source.substr(0, 2) )
    {
