                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    ((!singleQueue.empty()) && (!mergeQueue.empty())&&
    ((mergeQueue.front())->freq < (singleQueue.front())->freq))) 
    {
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while((singleQueue.size()+mergeQueue.size())>1)
    {
        ...
        newnode=new TreeNode(node1->freq.getFrequency()+node2->freq.getFrequency());
        newnode->left=node1;
        newnode->right=node2;
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        if (current_->left==NULL && current_->right==NULL)
        {
            ss<<current_->freq.getCharacter();
            current_=root_;
        }
    }
}
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
     ...
     */
    TreeNode* current_=current;
    if(current_->left==NULL && current_->right==NULL)
    {
        bfile.writeBit(1);
        bfile.writeByte(current_->freq.getCharacter());
        return;
    }
    ...
    if(current_->left)
    {
        writeTree(current_->left, bfile);
    }
    if(current_->right)
    {
        writeTree(current_->right, bfile);
    }
}
